...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card h3 { color: #4a5568; margin-bottom: 15px; font-size: 1.3em; display: flex; align-items: center; gap: 10px; }
        .status-indicator { width: 12px; height: 12px; border-radius: 50%; background-color: #48bb78; animation: pulse 2s infinite; }
        ...
        .metric-value { color: #2d3748; font-weight: 600; font-size: 1.1em; }
        .logs { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); border-radius: 15px; padding: 25px; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.1); }
        .logs h3 { color: #4a5568; margin-bottom: 15px; font-size: 1.3em; }
        .log-container { background: #1a202c; border-radius: 8px; padding: 15px; max-height: 300px; overflow-y: auto; font-family: 'Courier New', monospace; font-size: 0.9em; }
        .log-entry { color: #a0aec0; margin-bottom: 5px; line-height: 1.4; }
        ...
        .refresh-btn {
            ...
            transition: transform 0.2s ease; margin-top: 15px;
        }
        .refresh-btn:hover { transform: scale(1.05); }
        .auto-refresh { display: flex; align-items: center; gap: 10px; margin-top: 15px; color: #718096; }
...
<body>
    ...
    def serve_health_check(self):
        """Health check endpoint that verifies bot status"""
        try:
            # Try to import the bot instance from main.py
            from main import bot
            if bot.is_ready():
                response = {"status": "healthy", "bot": "online"}
                self.send_response(200)
            else:
                response = {"status": "unhealthy", "bot": "offline"}
                self.send_response(503)
        except Exception as e:
            logger.error(f"Health check failed: {str(e)}")
            response = {"status": "unhealthy", "bot": "offline", "error": str(e)}
            self.send_response(503)
...
class KeepAliveServer:
    def __init__(self, port=None, ping_url=None, ping_interval=300):
    ...
    def start(self):
        ...
        if self.ping_interval > 0:
            ...
            logger.info(f"Auto-ping started with {self.ping_interval}s interval")
...
def keep_alive(port=None, ping_url=None, ping_interval=300):
    """
    Start the enhanced keep alive server with auto-ping
    
    Args:
        port (int): Port to run the server on (default: from environment PORT or 8080)
