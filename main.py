import discord from discord.ext import commands from discord import app_commands from discord.ui import Button, View import os import random import asyncio import re from keep_alive import keep_alive  # Keep-alive added here

intents = discord.Intents.default() intents.guilds = True intents.messages = True intents.message_content = True intents.members = True

bot = commands.Bot( command_prefix="!", intents=intents, application_id=os.getenv("APPLICATION_ID") )

@bot.event async def on_ready(): print(f"✅ Logged in as {bot.user}") try: synced = await bot.tree.sync() print(f"✅ Synced {len(synced)} command(s).") except Exception as e: print(f"❌ Sync failed: {e}")

def parse_duration(duration_str): match = re.match(r'^(\d+)([hms])$', duration_str.lower()) if not match: return None value, unit = match.groups() value = int(value) if unit == 'h': return value * 3600 elif unit == 'm': return value * 60 elif unit == 's': return value return None

@bot.tree.command(name="giveaway", description="Create a giveaway with prize, duration, donor, and optional role requirement.") @app_commands.describe( prize="Giveaway prize", duration="Duration (e.g., 1h, 30m)", name="Donor name or who donated", role="(Optional) Role required to join" ) @app_commands.checks.has_permissions(manage_messages=True) async def giveaway( interaction: discord.Interaction, prize: str, duration: str, name: str, role: discord.Role | None = None ): seconds = parse_duration(duration) if seconds is None: await interaction.response.send_message("❌ Invalid duration format. Use formats like 1h, 30m, or 45s.", ephemeral=True) return
...
async def countdown_loop():
    remaining = seconds
    while remaining > 0 and not view.cancelled:
...
class GiveawayView(View): def init(self, required_role: discord.Role | None = None): super().init(timeout=None) self.joined_users = set() self.cancelled = False self.required_role = required_role self.message_id = None self.guild_id = None self.add_item(GiveawayJoinButton(self)) self.add_item(CancelButton(self)) self.add_item(RerollButton(self))

class GiveawayJoinButton(Button): def init(self, parent_view: GiveawayView): super().init(label="🎉 Join Giveaway", style=discord.ButtonStyle.green) self.parent_view = parent_view
...
class CancelButton(Button): def init(self, parent_view: GiveawayView): super().init(label="❌ Cancel Giveaway", style=discord.ButtonStyle.red) self.parent_view = parent_view
...
class RerollButton(Button): def init(self, parent_view: GiveawayView): super().init(label="🔄 Reroll Winner", style=discord.ButtonStyle.blurple) self.parent_view = parent_view
...
TOKEN = os.getenv("DISCORD_TOKEN") if TOKEN: bot.run(TOKEN) else: print("❌ Bot token not found. Please set DISCORD_TOKEN in your environment variables.")
