import discord from discord.ext import commands, tasks from discord import app_commands import asyncio import os import random from datetime import datetime, timedelta, timezone from dotenv import load_dotenv

load_dotenv()

intents = discord.Intents.default() intents.message_content = True token = os.getenv("TOKEN") guild_id = int(os.getenv("GUILD_ID"))
...
@bot.event async def on_ready(): await tree.sync(guild=discord.Object(id=guild_id)) print(f"Logged in as {bot.user}") check_giveaway_end.start()

@tree.command(name="giveaway", description="Start a giveaway", guild=discord.Object(id=guild_id)) @app_commands.describe(prize="What is the prize?", duration="Duration (e.g. 10s, 5m, 2h)") async def giveaway(interaction: discord.Interaction, prize: str, duration: str): await interaction.response.defer()
...
embed = discord.Embed(title="🎉 Giveaway! 🎉", description=f"**Prize:** {prize}\n**Ends:** <t:{int(end_time.timestamp())}:F>", color=0x00ff00)
...
@bot.event async def on_interaction(interaction: discord.Interaction): if interaction.type == discord.InteractionType.component and interaction.data['custom_id'] == 'join_giveaway': message_id = interaction.message.id user_id = interaction.user.id
...
@tasks.loop(seconds=120) async def check_giveaway_end(): now = datetime.now(timezone.utc) ended = []
